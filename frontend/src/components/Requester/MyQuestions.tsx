// components/MyQuestions.tsx
'use client';
import React, { useState } from 'react';
import { CheckCircle2, Clock, AlertCircle, ExternalLink, Copy, MoreHorizontal } from 'lucide-react';
...
const MyQuestions: React.FC<MyQuestionsProps> = ({ questions }) => {
      ...
      <div className="divide-y divide-gray-200">
        {questions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm">Submit your first oracle request to get started</p>
          </div>
        ) : (
                ...
                <div className="flex items-start space-x-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border transition-all duration-200 ${getStatusColor(question.status)}`}>
                        {question.status}
                      </span>
                    </div>
                    
                    <p className="text-sm text-gray-700 mb-3">{question.description}</p>
                      ...
                      <div className="flex items-center space-x-1">
                        {question.chains.map((chain, index) => (
                          <span key={index} className="inline-flex items-center">
                            {chain}
                            {index < question.chains.length - 1 && <span className="mx-1">→</span>}
                          </span>
                        ))}
                      </div>
                    </div>
              ...
              {selectedQuestion === question.id && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h5 className="text-sm font-medium text-gray-900 mb-2">Full Question</h5>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        <span className="font-medium text-gray-900">Networks:</span>
                        <p className="text-gray-600">{question.chains.join(', ')}</p>
                      </div>
                    </div>
