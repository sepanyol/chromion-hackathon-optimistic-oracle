...
const ActivityItem: React.FC<{ activity: ActivityItem }> = ({ activity }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'Proposed': return <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" />;
      case 'Challenged': return <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />;
      case 'Resolved': return <div className="w-2 h-2 bg-green-500 rounded-full" />;
  ...
  return (
    <div className="flex items-start space-x-3 py-4 hover:bg-gray-50 rounded-lg px-3 -mx-3 transition-colors duration-200 cursor-pointer group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center space-x-2 mt-1">
          ...
          <button 
            ...
            className="text-xs text-blue-600 hover:text-blue-800 font-mono flex items-center space-x-1 hover:bg-blue-50 px-2 py-1 rounded transition-all duration-200"
          >
      ...
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border transition-all duration-200 hover:scale-105 ${getStatusColor(activity.status)}`}>
        {activity.status}
      </span>
    </div>
  );
